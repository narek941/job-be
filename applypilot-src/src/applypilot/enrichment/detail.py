...
def scrape_detail_page(page, url: str, site: str | None = None) -> dict:
    ...
    # Tier 0 (Specialized Scrapers): Avoid slow browser if possible
    if site == "staff.am" or "staff.am" in url:
        from applypilot.discovery.staffam import scrape_staffam_job_details
        staff_res = scrape_staffam_job_details(url)
        if staff_res.get("full_description"):
            result.update(staff_res)
            result["tier_used"] = 0
        ...
        with sync_playwright() as p:
            ...
            for i, (url, title) in enumerate(jobs):
                ...
                if status in ("ok", "partial"):
                    stats[status] += 1
                    conn.execute(
                        "UPDATE jobs SET full_description = ?, application_url = ?, application_email = ?, "
                        "detail_scraped_at = ?, detail_error = NULL WHERE url = ?",
                        (result.get("full_description"), result.get("application_url"), result.get("application_email"), now, url),
                    )
                else:
                    stats["error"] += 1
                    conn.execute(
