...
def _build_cover_letter_prompt(profile: dict) -> str:
    ...
    base = f"""Write a cover letter for {sign_off_name}. The goal is to get an interview.
...
Start DIRECTLY with "Dear Hiring Manager," and end with the name."""
    ...
    arm = profile.get("armapply") or {}
    cl_lang = str(arm.get("cover_letter_language", "en") or "en").lower()
    if cl_lang == "hy":
        base = base + (
            "\n\n## LANGUAGE (ArmApply):\n"
            "Write the full letter in Armenian. Use «Պարոն/տիկին» or «Հարգելի ընտրող» style opening if "
            '"Dear Hiring Manager" in Armenian is preferred locally; otherwise keep professional tone.'
        )
    elif cl_lang in ("bilingual", "hy_en", "en_hy"):
        base = base + (
            "\n\n## LANGUAGE (ArmApply):\n"
            "Bilingual Armenian/English letter suitable for Armenian companies that use mixed language."
        )
    return base
