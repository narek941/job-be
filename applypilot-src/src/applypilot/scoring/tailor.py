...
def _build_tailor_prompt(profile: dict) -> str:
    ...
    lang = (profile.get("armapply") or {}).get("resume_language", "en") or "en"
    lang = str(lang).lower()
    language_block = ""
    if lang == "hy":
        language_block = (
            "\n\n## LANGUAGE (ArmApply):\n"
            "Write all tailored resume fields in Armenian (Հայերեն). "
            "Keep internationally known proper nouns and standard English tech terms where typical in Armenia."
        )
    elif lang in ("bilingual", "hy_en", "en_hy"):
        language_block = (
            "\n\n## LANGUAGE (ArmApply):\n"
            "Bilingual Armenian/English output for local employers: natural mix; "
            "technical terms in English when standard; narrative in Armenian where appropriate."
        )
