...
def get_daily_stats(conn: sqlite3.Connection | None = None, days: int = 7) -> list[dict]:
    """Return counts of job events (discovered, scored, applied) per day.
    ...
    Returns:
        List of dicts: [{"date": "YYYY-MM-DD", "discovered": int, "scored": int, "applied": int}, ...]
    """
    if conn is None:
        conn = get_connection()
    ...
    query = f"""
        ...
        SELECT 
            d.date,
            (SELECT COUNT(*) FROM jobs WHERE date(discovered_at) = d.date) as discovered,
            (SELECT COUNT(*) FROM jobs WHERE date(scored_at) = d.date) as scored,
            (SELECT COUNT(*) FROM jobs WHERE date(applied_at) = d.date) as applied,
            (SELECT COUNT(*) FROM jobs WHERE date(tailored_at) = d.date) as tailored
        FROM dates d
        ORDER BY d.date ASC
    """
...
def cleanup_old_jobs(conn: sqlite3.Connection | None = None, days: int = 30) -> int:
    """Delete jobs older than X days that were NOT applied to.

    This keeps the database size under control while preserving history
    of actual job applications.
    ...
    # We only delete if not applied_at is NULL (un-applied)
    # AND tailored_resume_path IS NULL (not even tailored)
    # OR if tailored but very old and never applied.
    # To be safe, we only delete jobs where applied_at is NULL.
    query = """
        DELETE FROM jobs
        WHERE (applied_at IS NULL OR applied_at = '')
          AND datetime(discovered_at) < datetime('now', ?)
    """
    res = conn.execute(query, (f"-{days} days",))
