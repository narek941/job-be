...
def _scrape_with_retry(kwargs: dict, max_retries: int = 2, backoff: float = 5.0):
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            err = str(e).lower()
            # If the country string is invalid, jobspy might be extracting it from the location.
            # We strip the country from the location string and force 'worldwide' or 'usa'.
            if "invalid country string" in err:
                log.warning("Invalid country rejected by JobSpy. Stripping country from location and forcing 'worldwide'...")
...
def _get_valid_country(country: str | None) -> str:
    ...
    # List of supported countries from JobSpy error message
    supported = {
        ...
        "oman", "pakistan", "panama", "pemu", "philippines", "poland", "portugal", 
    ...
    if c in ("arm", "am", "armenia"):
        return "worldwide" # JobSpy doesn't support AM; use worldwide to prevent irrelevant global matches
    if c == "ru":
        return "worldwide"
    return "worldwide"
...
def _run_one_search(
    ...
    if other_sites:
        country_val = _get_valid_country(defaults.get("country_indeed", "usa"))
        kwargs = {
            ...
            "country": country_val, # Some versions of jobspy use 'country' as the key
            "verbose": 0,
        }
        if s.get("remote"):
