...
== PORTAL-SPECIFIC HINTS (Armenia) ==
- Staff.am: 
  - Click "Apply" -> if a popup appears, it asks for login or shows the form.
  - Form usually has: "Upload CV", "Message to Recruiter" (paste Cover Letter here), and "Screening Questions". 
  - If it says "Fast Apply" or "Apply with Profile", click it. But ALWAYS ensure the tailored resume PDF is the one uploaded. Delete any pre-existing CV on the site if possible.
- HR.am: 
  - Often redirects to external ATS (Workday, Lever, etc.) or a simple form. Follow the general rules.
