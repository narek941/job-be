...
def _run_discover(workers: int = 1) -> dict:
    ...
    # staff.am (Armenia) — HTTP listing parse
    console.print("  [cyan]staff.am (Armenia)...[/cyan]")
    try:
        from applypilot.discovery.staffam import run_staffam_discovery
        staff_cfg = (load_search_config() or {}).get("staff_am") or {}
        if staff_cfg.get("enabled", True):
            max_pages = int(staff_cfg.get("max_pages_per_keyword", 2))
            run_staffam_discovery(max_pages_per_keyword=max_pages)
            stats["staffam"] = "ok"
        else:
            stats["staffam"] = "skipped"
    except Exception as e:
        log.error("staff.am discovery failed: %s", e)
        console.print(f"  [red]staff.am error:[/red] {e}")
        stats["staffam"] = f"error: {e}"

    # Публичные Telegram-каналы (t.me/s/...) — см. discovery/telegram_channels.py
    console.print("  [cyan]Telegram channels (public web feed)...[/cyan]")
    try:
        from applypilot.discovery.telegram_channels import run_telegram_channel_discovery
        tg_cfg = (load_search_config() or {}).get("telegram_channels") or {}
        if tg_cfg.get("enabled", False):
            max_pages = int(tg_cfg.get("max_pages_per_channel", 1))
            stats["telegram"] = run_telegram_channel_discovery(max_pages_per_channel=max_pages)
        else:
            stats["telegram"] = "skipped"
    except Exception as e:
        log.error("Telegram channel discovery failed: %s", e)
        console.print(f"  [red]Telegram channels error:[/red] {e}")
        stats["telegram"] = f"error: {e}"

    # Djinni (Ukraine/Remote)
    console.print("  [cyan]Djinni (Ukraine/Remote)...[/cyan]")
    try:
        from applypilot.discovery.djinni import run_djinni_discovery
        run_djinni_discovery(max_pages_per_keyword=2)
        stats["djinni"] = "ok"
    except Exception as e:
        log.error("Djinni discovery failed: %s", e)
        console.print(f"  [red]Djinni error:[/red] {e}")
        stats["djinni"] = f"error: {e}"
